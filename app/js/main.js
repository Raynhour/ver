$('document').ready(function(){
    ...
    if( windowWidth <= 767 ) {
        $('body.home .sec_centr, body.home .sec_courses').hide();
    }

    $('#ship-to-different-address').hide();

    var scourier = $('#shipping_method_0_free_shipping-3');
    var ssamovivoz = $('#shipping_method_0_local_pickup-4');
    var scity = $('#billing_city_field');
    var soblast = $('#billing_state_field');
    var saddress = $('#billing_address_1_field');

    if ( scourier.is(':checked') ) {
        scity.delay(500).hide(300);
        soblast.delay(500).hide(300);
    }
    if ( ssamovivoz.is(':checked') ) {
        scity.delay(500).hide(300);
        soblast.delay(500).hide(300);
        saddress.delay(500).hide(300);
    }

    $('body').change( function() {
        var courier = $(this).find('#shipping_method_0_free_shipping-3');
        var samovivoz = $(this).find('#shipping_method_0_local_pickup-4');
        var city = $(this).find('#billing_city_field');
        var oblast = $(this).find('#billing_state_field');
        var address = $(this).find('#billing_address_1_field');

        if ( courier.is(':checked') ) {
            city.delay(500).hide(300);
            oblast.delay(500).hide(300);
        }
        if ( samovivoz.is(':checked') ) {
            city.delay(500).hide(300);
            oblast.delay(500).hide(300);
            address.delay(500).hide(300);
        }
    });
